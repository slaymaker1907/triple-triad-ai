...
@FunctionalInterface
public interface AscensionTransform extends BiFunction<DeployedCard, Integer, DeployedCard>
{
	/**
	...
	/**
	 * This function returns the input card with its stats increased by the amount input
	 * @param card The card just played.
	 * @param amount The amount to increase the card's stats by.
	 * @return The input card with its new stats.
	 */
	public static DeployedCard ascension(DeployedCard card, int amount)
	{
		return new DeployedCard(card.card.increaseAllStats(amount), card.row, card.col);
	}
	 ...
	 * @param card The card just played.
	 * @param The amount to decrease the card's stats by.
	 * @return The input card with its new stats.
	 */
