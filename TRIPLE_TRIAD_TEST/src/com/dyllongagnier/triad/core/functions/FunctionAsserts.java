...
public class FunctionAsserts
{
	...
	/**
	 * This method runs a number of tests over the input functions to determine
	 * if they are equal, throwing an AssertionError otherwise.
	 * 
	 * @param expected
	 ...
	 *            The actual function.
	 */
	public static void assertEquals(DeployedCardComparator expected,
			DeployedCardComparator actual)
	{
		...
		Assert.assertEquals(expected.apply(middleDodo, upMidGael),
				actual.apply(middleDodo, upMidGael));

		// Check to see if it neither captures if one is equality.
		Assert.assertEquals(expected.apply(middleDodo, upMidSpriggan),
				actual.apply(middleDodo, upMidSpriggan));
		Assert.assertEquals(expected.apply(upMidSpriggan, middleDodo),
				actual.apply(upMidSpriggan, middleDodo));

		// Check an ace comparison with a 1.
		Assert.assertEquals(expected.apply(upMidGael, middleNid),
				actual.apply(upMidGael, middleNid));
	}
	 ...
	 *            The actual function.
	 */
	public static void assertEquals(AscensionTransform expected,
			AscensionTransform actual)
	{
	 ...
	 *            The actual function.
	 */
	public static void assertEquals(CardPlayFunction expected,
			CardPlayFunction actual)
	{
		// throw new
		// UnsupportedOperationException("This method is not yet implemented.");
		...
		// This field should trigger plus, same, and combo if they are in
		// effect.
		Field actualField = new Field(DeployedCardComparator::regularCompare,
				actual);
		...
		Assert.assertEquals(expected.updateField(actualField, center,
				DeployedCardComparator::regularCompare), expected.updateField(
				expectedField, center, DeployedCardComparator::regularCompare));
	}
	...
	private static Field getSamePlusComboTriggerField(Field start)
	{
		...
		return start.playCard(new DeployedCard(opponentOne, 0, 0))
				...
				.playCard(new DeployedCard(opponentOne, 2, 2)).playCard(north)
				.playCard(east).playCard(south).playCard(west);
	}
}
